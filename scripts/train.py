...
def _import_data(metadata_path=METADATA_PATH, ecg_source_dir=ECG_SOURCE_DIR):
    ...
    test_df = df[
        (df.set == 'Internal validation') &
        (df.qc == 'Good')
    ]
    ext_df = df[
        (df.set == 'External validation') &
        (df.qc == 'Good')
    ]
...
def _train_and_save_model(experiment, model, x_train, y_train, x_test, y_test, model_save_dir=MODEL_SAVE_DIR):
    '''
...
def _eval_model(experiment, model, x_test, y_test, x_ext, y_ext):
    ...
    experiment.log_confusion_matrix(y_true=np.argmax(y_test,1), 
    ...
    experiment.log_confusion_matrix(y_true=np.argmax(y_ext,1), 
    ...
    def roc_curve(y_true, y_probas, title):
        ...
        plt.text(fpr[ix]+0.02,tpr[ix]-0.08, f"Sen {round(tpr[ix],2)}, Spe {round(1-fpr[ix],2)}")
        plt.title(title)
    ...
    def pr_curve(y_true, y_probas, title):
        ...
        fig, ax = plt.subplots()
        metrics.PrecisionRecallDisplay.from_predictions(y_true, y_probas, 
            ax=ax, name=f"F1 score = {f1_score:.3f}")
    ...
    roc_curve(y_test_true, y_test_probas, "LQTS1/2 carrier status (Internal Validation)")
    ...
    y_pred = np.apply_along_axis(lambda a: a[0]/(a[0]+a[1]), 1, y_test_pred[lqt_test_index,1:3][0])
    ...
    y_pred = np.apply_along_axis(lambda a: a[0]/(a[0]+a[1]), 1, y_ext_pred[lqt_ext_index,1:3][0])
...
if __name__ == '__main__':
    ...
    for experiment in opt.get_experiments():
        ...
        with experiment.train():
            model, save_path = _train_and_save_model(experiment, model, x_train, y_train, x_test, y_test)
